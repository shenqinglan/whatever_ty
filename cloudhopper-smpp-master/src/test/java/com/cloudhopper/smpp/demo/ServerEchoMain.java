...
public class ServerEchoMain {
    ...
    public static class TestSmppSessionHandler extends DefaultSmppSessionHandler {
        ...
        public PduResponse firePduRequestReceived(PduRequest pduRequest) {
            final SmppSession session = sessionRef.get();

            PduResponse response = pduRequest.createResponse();
            
            if (pduRequest.getCommandId() == SmppConstants.CMD_ID_SUBMIT_SM) {
                SubmitSm mt = (SubmitSm) pduRequest;
                final Address mtSourceAddress = mt.getSourceAddress();
                final Address mtDestinationAddress = mt.getDestAddress();
                final byte dataCoding = mt.getDataCoding();
                final byte[] shortMessage = mt.getShortMessage();
					...
					public void run() {
						try {
							...
							sendMoMessage(
									...
									mtSourceAddress, 
									"E1354C10890010120123412340123456789012244D01024E0200004F10A0000005591010FFFFFFFF890000130014081122334455667788".getBytes(), 
									dataCoding);
					...
					public void run() {
						try {
							...
							sendMoMessage(
									...
									mtDestinationAddress, 
									mtSourceAddress, 
									"F1354C10890010120123412340123456789012244D01024E0200004F10A0000005591010FFFFFFFF890000130014081122334455667788".getBytes(), 
									dataCoding);
						} catch (InterruptedException e) {
							e.printStackTrace();
						}
