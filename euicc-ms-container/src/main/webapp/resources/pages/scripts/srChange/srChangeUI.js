				...
				.init({
					...
					onDataLoad : function(grid) {
						$(".change").click(function() {
							...
							if (!confirm("是否确定SR-CHANGE该数据信息？")) {
								return false;
							}
							$.ajax({
								type : 'POST',
								url : ctx + '/srChange/change',
								data : {
									eid : fieldId
								},
								dataType : 'json',
...
function setFormStatus(type) {
	...
	if ("edit" == type) {
		$("#tag").val("update");
		$("#srChangeModalLabel").html("编辑");//请完善编辑内容
	} else if ("add" == type) {
		$("#tag").val("add");
		$("#srChangeModalLabel").html("新建");//请完善新建内容
	}
};
...
function getPremission(data){
	var p = "<shiro:hasPermission name='srChange:change'><a class='btn default change btn-xs blue' name='"+data+"'><i class='fa fa-edit'></i>SR切换 </a></shiro:hasPermission>";
	return p;
}
