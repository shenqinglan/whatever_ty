		...
		installProfileGrid
				.init({
					...
					onDataLoad : function(installProfileGrid) {
						...
						$(".updateconnetivityPara").click(
								function() {
									$('#installProfileTableModule').modal('hide');
									var fieldId = $(this).attr("name");
									$.ajax({
										type : 'POST',
										url : ctx + '/installProfile/getCurrSmsCenterNo',
										data : {
											iccid : fieldId
										},
										dataType : 'json',
										success : function (data) {
											smsCenterN = prompt("请输入短信中心号码",data)
											console.log(smsCenterN);
											
   										    judgeInput(smsCenterN,fieldId);
											
												
										}
									});
								});
						function judgeInput(smsCenterN,fieldId){
							if(smsCenterN == ""){
								alert("短信中心号码不能为空!");
							}else if(isNaN(smsCenterN)){
								alert("短信中心号码仅能为数字!");
							}else if(smsCenterN.length != 13){
								alert("短信中心号码长度错误!");
							}else if(smsCenterN&&smsCenterN != ""){
								setSmsCenterNum(smsCenterN,fieldId);
							}else{
								return;
							}
						}
						function setSmsCenterNum(smsCenterNo,fieldId){
							$.ajax({
								type : 'POST',
								url : ctx + '/installProfile/updateconnetivityPara',
								timeout : 180000,
								data : {
									iccid : fieldId,
									eid : $('#cardEid').val(),
									smsCenterNo : smsCenterNo

								},
								dataType : 'json',
								success : function(data) {
									if(data.success){
										toastr['success']("SCP03更新连接参数成功",
											"系统提示");
										$('#installProfileTableModule').modal('show');
										installProfileGrid.getDataTable().ajax.reload();
										installProfileGrid.clearAjaxParams();
						...
						$(".updateSrAddress").click(
								function() {
									$('#installProfileTableModule').modal('hide');
									$.ajax({
										type : 'POST',
										url : ctx + '/installProfile/getCurrSrAddressNo',
										data : {
											eid : $('#cardEid').val(),
										},
										dataType : 'json',
										success : function (data) {
											srAddressN = prompt("请输入短信目的地址",data)
											console.log(srAddressN);
											judgeInputData(srAddressN);
										}
									});
								});
						function judgeInputData(srAddressN){
							if(srAddressN == ""){
								alert("短信目的地址不能为空!");
							}/*else if(isNaN(srAddressN)){
								alert("短信目的地址仅能为数字!");
							}else if(srAddressN.length != 10){
								alert("短信目的地址长度错误!");
							}*/else if(srAddressN&&srAddressN != ""){
								setSrAddress(srAddressN);
							}else{
								return;
							}
						}
						function setSrAddress(srAddressNo){
							$.ajax({
								type : 'POST',
								url : ctx + '/installProfile/updateSrAddress',
								timeout : 180000,
								data : {
									eid : $('#cardEid').val(),
									srAddress : srAddressNo
								},
								dataType : 'json',
								success : function(data) {
									if(data.success){
					...
					dataTable : {
						...
						,{'name':'ISD-P-AID','orderable' : false,'targets' : [3]}
						,{'name':'ICCID','orderable' : false,'targets' : [4]}],
						'columns' : [
								{
									'title' : 'ICCID',
									'field' : 'iccid'
...
function getInstallPremission(data){
	//var p = "<shiro:hasPermission name='installProfile:install'><a class='btn default install btn-xs blue' name='"+data+"'><i class='fa fa-edit'></i>下载安装 </a></shiro:hasPermission>";
	 var p = "<shiro:hasPermission name='installProfile:createIsdP'><a class='btn default createIsdP btn-xs blue' name='"+data+"'><i class='fa fa-edit'></i>创建ISD-P </a></shiro:hasPermission>";
	    ...
	    p += "<shiro:hasPermission name='installProfile:updateconnetivityPara'><a class='btn default updateconnetivityPara btn-xs blue' name='"+data+"'><i class='fa fa-edit'></i>SCP03更新连接参数 </a></shiro:hasPermission>";
	    p += "<shiro:hasPermission name='installProfile:updateSrAddress'><a class='btn default updateSrAddress btn-xs blue' name='"+data+"'><i class='fa fa-edit'></i>更新SMSR地址参数 </a></shiro:hasPermission>";
	return p;
}
