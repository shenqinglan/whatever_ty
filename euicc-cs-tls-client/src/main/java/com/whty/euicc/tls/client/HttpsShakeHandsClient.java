...
public class HttpsShakeHandsClient {
	...
	public String clientTestByRsp(String str,String ip,int port) throws  Exception{
		System.out.println("********begin*********");
		...
		if (flag == 0) {
			resultData = doHttpPostByRsp(tlsHankUtils, socket,str);
			System.out.println("shakeHands success");
		} else {
			System.out.println("shakeHands error ,error code " + flag);
			return null;
	...
	private String doHttpPostByRsp(AbstractCTlsHank tlsHankUtils, Socket socket,String str)
			throws IOException {
		firstSendPostByRsp(tlsHankUtils, socket,str);
		byte[] strserverResponse = tlsHankUtils.receiveServerDataByRsp(socket);
		String result = new String(strserverResponse);
