...
public class Command implements SMSConstants {
	...
	public String toString(boolean isFormate) {
		...
		if ((this.SPI.equals("")) && (this.KIc.equals(""))
			...
			try {
					res = orgCmdUpForDes(isFormate,this.SPI, this.KIc, this.KID, this.TAR, this.CNTR,
						this.plainString,this.encryptKey,this.ccKey);
	...
	public String orgCmdUpForDes(boolean isFormate,String spi, String kic, String kid, String tar,
			String cntr, String plainData,String encryKey,String ccKey) throws Exception{
		...
		if (checkKid(kidByte, spiByte)) {
			/*
			 * MAC计算输入数据（遵循标准0348）：CPL,CHL,SPI,KIC,KID,TAR,CONTR,PCNTR, 命令类型、命令长度、 命令数据的所有字段。
			 */
			String needMacStr = CPL + this.CHL + spi + kic + kid + tar + cntr
					+ this.PCNTR + paddedStr;
     		this.RCCCDS = calcRCCCDS(this.RCCCAlg, needMacStr,ccKey);
		...
		if (checkKic(kicByte, spiByte)) {
			...
			/*
			 * 加密算法输入数据（遵循标准0348）：CNTR,PCNTR,MAC, 命令类型、命令长度、 命令数据的所有字段
			 */
			String needEnCryStr = cntr + this.PCNTR + this.RCCCDS + paddedStr;
			this.securedString = enCrypt(this.cipherAlg, needEnCryStr,encryKey);
			...
			lastcommand = this.CHI + this.CHL + this.SPI + this.KIc + this.KID + this.TAR
					+ cntr + this.PCNTR + this.RCCCDS + this.plainString;
		}
...
//			String keyStrForKid = getKidString(kidByte, spiByte);
			String keyStrForKid ="11223344556677889910111213141516";
			String asString = respData.substring(6, 38) ;
			String pcnString = respData.substring(36, 38) ;
			String ad = respData.substring(54) ;
			//CPL + this.CHL + spi + kic + kid + tar + cntr+ this.PCNTR + paddedStr;
			int pcntr1 = StringUtil.hexToByte(pcnString);
			String paddedStr = ad
					+ "0000000000000000".substring(0, pcntr1 * 2);

			String needMac = asString +paddedStr;
			AesCmac aesMac = new AesCmac();
			String resultMac = aesMac.calcuCmac(needMac, keyStrForKid) ;
			if(!resultMac.equals(mac)){
				return error;
	...
	public String calcRCCCDS(int RCCCAlg, String hexData,String keyStr) {
		String res = "";

		//TODO:从数据库获取密钥
//		String keyStr = "52DABF691AFD0E2B540CA6A09DEE3BF9";//kid
	...
	private String enCrypt(int cipherAlg, String hexData,String keyStr) {
		String res = hexData;
		byte[] data = StringUtil.hexToBuffer(hexData);

		//TODO:从数据库获取密钥
//		String keyStr = "D9BE655005BEB775324D53DEA699F13A";//kic
