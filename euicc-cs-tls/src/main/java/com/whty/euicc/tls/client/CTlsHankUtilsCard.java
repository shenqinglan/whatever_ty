...
import com.whty.euicc.tls.server.TlsCipherSuite;
import com.whty.euicc.tls.server.TlsCipherSuiteManager;
import com.whty.euicc.tls.server.TlsNullCipherSuite;

public class CTlsHankUtilsCard extends AbstractCTlsHank{
	PSK aPsk;
	...
	private String clientKeyExchange() throws IOException {
		String vlauePskID = getPskKey(FileName_pskKeyIni, pskKeyIni_PSKID,
				"0140");
		//vlauePskID = Util.byteArrayToHexString(Util.toByteArray(vlauePskID), "");
		
		System.out.println("vlauePskID >>" + vlauePskID);
...
//					+ Util.byteArrayToHexString(checksum, " "));
	...
	public void sendEncryptData(Socket s,byte[] dataByte) throws IOException{
		...
		byte[] encryptData = encryptApplicationDataByEuicc(dataByte, 0, dataByte.length);	
	...
	public byte[] receiveServerData(Socket s) throws IOException{
		...
		System.out.println("postRequestFinal >>" + strserverResponse);
	...
	protected byte[] decryptApplicationData(byte[] hex) throws IOException {
		...
		System.out.println("解密后的报文为:"+new String(buffer));
		return buffer;
	}
	
	private byte[] decryptApplicationDataByEuicc(byte[] serverResponseToByte) throws IOException {
		byte[] buffer = decryptApplicationData(serverResponseToByte);
	...
	private byte[] encryptApplicationDataByEuicc(byte[] message, int offset, int len)
			throws IOException {
		byte[] writeMessage = encryptApplicationData(message,offset,len);
		String hexStr = Util.byteArrayToHexString(writeMessage, "");
		return SocketUtils.hexToBuffer(hexStr);
	}
	
	protected byte[] encryptApplicationData(byte[] message, int offset, int len)
			throws IOException {
