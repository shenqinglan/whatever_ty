...
/**
 * @ClassName BindTraAndRecForOneMain
 * @author Administrator
 * @date 2017-1-23 下午3:23:32
 * @Description TODO(这里用一句话描述这个类的作用)
 */
public class BindTraAndRecForOneMain {
	...
	public static void main(String[] args) {
		final BindTraAndRecForOneMain client = new BindTraAndRecForOneMain();
	...
	public BindTraAndRecForOneMain() {
		...
		session = new BasicSession(conn, "BindTraAndRecForOne");
	...
	private void bindReceiver() {
		...
		bindReceiver.setSequenceNumber(session.nextSequenceNumber());
		try {
			session.send(bindReceiver);
