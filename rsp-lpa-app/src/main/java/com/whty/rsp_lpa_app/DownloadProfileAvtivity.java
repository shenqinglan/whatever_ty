...
import com.whty.rsp_lpa_app.tls.TlsManager;
import com.whty.rsp_lpa_app.utils.ApduUtils;
 ...
 *
 */
public class DownloadProfileAvtivity extends BaseActivity{
	
	private final String TAG = "DownloadProfileAvtivity";
	...
	private void downloadAndInstall() {
		...
		new AsyncTask<String, Integer, String>() {
			...
			protected String doInBackground(String... params) {
				...
				return TlsManager.getInstance().initiateAuthentication(eUICCChallenge, euiccinfo1, smdpAdress, params[0], Integer.parseInt(params[1]));
			}
...
//		}
		...
		new AsyncTask<String, Integer, String>() {
			...
			protected String doInBackground(String... params) {
				//调用ES9+.authenticateClient
				return TlsManager.getInstance().authenticateClient(transactionId, authenticateServerResponse.getEuiccSigned1(), 
			...
			protected void onPostExecute(String result) {
				super.onPostExecute(result);
				checkProfile(result);
				Toast.makeText(DownloadProfileAvtivity.this, "双向认证成功", 0).show();
...
//		}
		new AsyncTask<String, Integer, String>() {
			...
			protected String doInBackground(String... params) {
				...
				return TlsManager.getInstance().getBoundProfilePackage(transactionId, 
			...
			protected void onPostExecute(String result) {
				...
				Toast.makeText(DownloadProfileAvtivity.this, "下载成功", 0).show();
			}
		}.execute(GlobalParams.DP_IP_PORT);
	}
		...
		try {
			...
			new AsyncTask<String, Integer, String>() {
				...
				protected String doInBackground(String... params) {
					...
					return TlsManager.getInstance().handleNotification(profileInstallationResultData, euiccSignPIR, params[0], Integer.parseInt(params[1]));
				}
				...
				protected void onPostExecute(String result) {
					...
					Toast.makeText(DownloadProfileAvtivity.this, "安装成功", 0).show();
					finish();
				}
			}.execute(GlobalParams.DP_IP_PORT);
		} catch (Exception e){
	...
	private void userRejection(final String transactionId, final String reason) {
		String cancelSessionResponse = cancelSessionForEuicc(transactionId, reason);
		...
		new AsyncTask<String, Integer, String>() {
			...
			protected String doInBackground(String... params) {
				//调用ES9+.cancelSession
				return TlsManager.getInstance().cancelSession(transactionId, euiccCancelSessionSigned, euiccCancelSessionSignature, params[0],Integer.parseInt(params[1]));
			}
			
			@Override
			protected void onPostExecute(String result) {
				super.onPostExecute(result);
				Toast.makeText(DownloadProfileAvtivity.this, "用户取消，原因："+reason, 1).show();
				finish();
			}
		}.execute(GlobalParams.DP_IP_PORT);
	}
	...
	private String getDpAddress(String downloadType, String result) {
		...
		if ("3".equals(downloadType)) {//TODO
			//cardMain.getEuiccConfiguredAddresses(apdu);
	...
	private String getEuiccInfo1() {
		...
		if ("".equals(euiccInfo)) {
			String apdu = ApduUtils.getEUICCInfo1Apdu();
...
//		return cardMain.prepareDownload(apdu);
	 ...
	 * @return
	 */
	private String loadBoundProfilePackage(String data) {
//		String apdu = "";//TODO
