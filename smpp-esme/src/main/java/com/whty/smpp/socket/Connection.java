...
/**
 * 短信连接字符串 User: gaudi.gao Date: 14-6-17 Time: 上午10:20 To change this template
...
public abstract class Connection implements java.io.Closeable {
	...
	protected void onError(String message) {
		log.error(String.format("%s host=%s,port=%d", message, this.getHost(),
				this.getPort()));
	}
	...
	protected void onConnect() {
		log.info(String.format("socket connect success host=%s,port=%d %tc%n",
				this.getHost(), this.getPort(), new Date()));
		if (session == null) {
	...
	protected void onDisconnect() {
		log.info(String.format(
				"socket disconnect success host=%s,port=%d %tc%n",
				this.getHost(), this.getPort(), new Date()));
		if (autoReconnect) {
			try {
				Thread.sleep(1000);
	...
	protected void onClose() {
		log.info(String.format("socket close success host=%s,port=%d %tc%n",
				this.getHost(), this.getPort(), new Date()));
	}
			...
			public void run() {
				...
				if (isConnected()) {
					try {
						...
						onError("socket connection receive msg error: "
								+ ex.getMessage(), ex);
					}
				}
			}
