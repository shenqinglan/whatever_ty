...
import com.whty.euicc.rsp.profile.profileBean.TLVBean;

/**
 * 生成Segmented Bound Profile Package
 * 
...
public class ProfileUtil {
	 ...
	 * @return
	 */
	public String[] generateSBPP(String BPP){
		...
		String[] str1 = headAndInitChannel(BPP);
		for(int i=0;i<str1.length;i++){
			array.add(str1[i]);
			endOfSeg += str1[i].length();
		}
		String[] str2 = configIsdp(BPP.substring(endOfSeg));
		for(int i=0;i<str2.length;i++){
			array.add(str2[i]);
			endOfSeg += str2[i].length();
		}
		...
		String[] str4 = ppk(BPP.substring(endOfSeg));
		for(int i=0;i<str4.length;i++){
			array.add(str4[i]);
			endOfSeg += str4[i].length();
		}
	...
	private String[] headAndInitChannel(String data) {
		...
		//对字符串进行限定长度的分包
		return lengthLimit(head + initChannel);
	}
	...
	private String[] metaData(String data){
		...
		//对head进行限定长度分包
		String[] headOfMetadata = lengthLimit(head);
		for(int i=0,n = headOfMetadata.length;i<n;i++){
			array2.add(headOfMetadata[i]);
		}
		//对body进行限定长度分包
		for(int i=0,n = body.length;i<n;i++){
			String[] bodyOfMetadata = lengthLimit(body[i]);
			for(int j=0,m = bodyOfMetadata.length;j<m;j++){
				array2.add(bodyOfMetadata[j]);
			}
		}
		return (String[]) array2.toArray(new String[0]);
	}
	/**
	...
	private String[] ppp(String data){
		...
		ArrayList<String> array2 = new ArrayList<String>();
		//对head进行限定长度分包
		String[] headOfPPP = lengthLimit(head);
		for(int i=0,n = headOfPPP.length;i<n;i++){
			array2.add(headOfPPP[i]);
		}
		//对body进行限定长度分包
		for(int i=0,n = body.length;i<n;i++){
			String[] bodyOfPPP = lengthLimit(body[i]);
			for(int j=0,m = bodyOfPPP.length;j<m;j++){
				array2.add(bodyOfPPP[j]);
			}
		}
		return (String[]) array2.toArray(new String[0]);
	}
	 ...
	 * @return
	 */
	private TLVBean selectTlv(String inputData ,String beginTag ,String endTag) {
